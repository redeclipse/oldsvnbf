...
FVAR(ragdollgravity, 0, 1.5f, 1000);
...
void ragdolldata::move(dynent *pl, float ts)
{
    ...
    loopv(skel->verts)
    {
        ...
        dpos.mul(pow((pl->inliquid ? ragdollwaterfric : 1.0f) * (v.collided ? ragdollgroundfric : ragdollairfric), ts*1000.0f/ragdolltimestepmin)*expirefric);
