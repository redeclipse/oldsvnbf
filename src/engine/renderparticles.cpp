...
void makeparticle(const vec &o, vector<int> &attr)
{
	switch(attr[0])
	{
		...
		case 6: //meter, metervs - <percent> <rgb> <rgb2>
		{
			...
			part_icon(o, textureload(hud::progresstex, 3), 2.5f, 1, 0, 0, 1, attr[3], length, 1-length); // fall through
		}
		case 5:
		{
			float length = clamp(attr[1], 0, 100)/100.f;
			part_icon(o, textureload(hud::progresstex, 3), 2.5f, 1, 0, 0, 1, attr[2], 0, length);
			break;
		}
