...
struct md3 : vertmodel
{
    ...
    bool loaddefaultparts()
    {
        ...
        mdl.model = this;
        mdl.index = 0;
        defformatstring(name1)("models/%s/tris.md3", loadname);
    ...
    bool load()
    {
        ...
        formatstring(md3dir)("models/%s", loadname);
...
void md3link(int *parent, int *child, char *tagname, float *x, float *y, float *z)
{
    ...
    if(!loadingmd3->parts[*parent]->link(loadingmd3->parts[*child], tagname, vec(*x, *y, *z))) conoutf("\frcould not link model %s", loadingmd3->loadname);
}
