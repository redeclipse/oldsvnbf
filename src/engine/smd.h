...
struct smd : skelmodel
{
    ...
    bool loaddefaultparts()
    {
        ...
        smdadjustments.setsizenodelete(0);
        const char *fname = loadname + strlen(loadname);
        do --fname; while(fname >= loadname && *fname!='/' && *fname!='\\');
        fname++;
        defformatstring(meshname)("models/%s/%s.smd", loadname, fname);
    ...
    bool load()
    {
        ...
        formatstring(smddir)("models/%s", loadname);
        defformatstring(cfgname)("models/%s/smd.cfg", loadname);
...
{
    if(!loadingsmd) { conoutf("\frnot loading an smd"); return; }
    formatstring(smddir)("models/%s", name);
}
...
{
    if(!loadingsmd || loadingsmd->parts.empty()) { conoutf("\frnot loading an smd"); return; }
...
void smdanim(char *anim, char *animfile, float *speed, int *priority)
{
    ...
    if(anims.empty()) conoutf("\frcould not find animation %s in %s", anim, loadingsmd->loadname);
    else 
    {
        ...
        if(!sa) conoutf("\frcould not load smd file %s", filename);
        else loopv(anims)
        {
...
{
    if(!loadingsmd) { conoutf("\frnot loading an smd"); return; }
    if(!loadingsmd->parts.inrange(*parent) || !loadingsmd->parts.inrange(*child)) { conoutf("\frno models loaded to link"); return; }
    if(!loadingsmd->parts[*parent]->link(loadingsmd->parts[*child], tagname, vec(*x, *y, *z))) conoutf("\frcould not link model %s", loadingsmd->loadname);
}
...
COMMANDN(0, smddir, setsmddir, "s");
COMMAND(0, smdload, "ss");
COMMAND(0, smdtag, "ss");
COMMAND(0, smdpitch, "sffff");
COMMAND(0, smdadjust, "sffffff");
COMMAND(0, smdskin, "sssff");
COMMAND(0, smdspec, "si");
COMMAND(0, smdambient, "si");
COMMAND(0, smdglow, "si");
COMMAND(0, smdglare, "sff");
...
COMMAND(0, smdshader, "ss");
COMMAND(0, smdscroll, "sff");
COMMAND(0, smdanimpart, "s");
COMMAND(0, smdanim, "ssfi");
COMMAND(0, smdlink, "iisfff");
COMMAND(0, smdnoclip, "si");            
