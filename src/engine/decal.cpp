...
VARF(maxdecaltris, 1, 4096, INT_MAX-1, initdecals());
...
struct decalrenderer
{
    ...
    void adddecal(const vec &center, const vec &dir, float radius, const bvec &color, int info)
    {
        ...
        if(dbgdec)
        {
            ...
            conoutf("\fytris = %d, verts = %d, total tris = %d", nverts/3, nverts, (maxverts - 3 - availverts)/3);
        }
...
{
    decalrenderer("particles/scorch", DF_ROTATE, 500),
