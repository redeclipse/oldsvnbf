...
struct iqm : skelmodel
{
    ...
    bool loaddefaultparts()
    {
        ...
        fname++;
        defformatstring(meshname)("models/%s/%s.iqm", loadname, fname);
    ...
    bool load()
    {
        ...
        formatstring(iqmdir)("models/%s", loadname);
        defformatstring(cfgname)("models/%s/iqm.cfg", loadname);
...
{
    if(!loadingiqm) { conoutf("\frnot loading an iqm"); return; }
    formatstring(iqmdir)("models/%s", name);
}
...
{
    if(!loadingiqm || loadingiqm->parts.empty()) { conoutf("\frnot loading an iqm"); return; }
...
void iqmanim(char *anim, char *animfile, float *speed, int *priority)
{
    ...
    else 
    {
        ...
        if(!sa) conoutf("\frcould not load iqm anim file %s", filename);
        else loopv(anims)
        {
...
void iqmlink(int *parent, int *child, char *tagname, float *x, float *y, float *z)
{
    ...
    if(!loadingiqm->parts[*parent]->link(loadingiqm->parts[*child], tagname, vec(*x, *y, *z))) conoutf("\frcould not link model %s", loadingiqm->loadname);
}
...
COMMANDN(0, iqmdir, setiqmdir, "s");
COMMAND(0, iqmload, "ss");
COMMAND(0, iqmtag, "ss");
COMMAND(0, iqmpitch, "sffff");
COMMAND(0, iqmadjust, "sffffff");
COMMAND(0, iqmskin, "sssff");
COMMAND(0, iqmspec, "si");
COMMAND(0, iqmambient, "si");
COMMAND(0, iqmglow, "si");
COMMAND(0, iqmglare, "sff");
...
COMMAND(0, iqmshader, "ss");
COMMAND(0, iqmscroll, "sff");
COMMAND(0, iqmanimpart, "s");
COMMAND(0, iqmanim, "ssfi");
COMMAND(0, iqmlink, "iisfff");
COMMAND(0, iqmnoclip, "si");            
