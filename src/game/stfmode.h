	...
	void endcheck()
	{
		...
		loopi(numteams(gamemode, mutators))
		{
			...
			if(findscore(lastteam).total >= maxscore)
			{
				...
				sendf(-1, 1, "ri2s", SV_ANNOUNCE, S_GUIBACK, CON_INFO, "\fcsecure limit has been reached!");
		...
		if(GVAR(stffinish))
		{
			...
			if(lastteam)
			{
				...
				sendf(-1, 1, "ri2s", SV_ANNOUNCE, S_GUIBACK, CON_INFO, "\fcall flags have been secured!");
