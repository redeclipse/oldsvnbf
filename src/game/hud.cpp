...
namespace hud
{
	...
	int drawhealth(int x, int y, int s, float blend)
	{
		...
		if(game::player1->state == CS_ALIVE)
		{
			...
			{
				float len = 1.f-clamp(game::player1->impulse[IM_METER]/float(m_speedtime(impulselength)), 0.f, 1.f);
				...
				if(inventoryimpulse >= 2)
				{
					...
					draw_textx("%s%d%%", x+sw/2, y-sy-sw/2-FONTH/2, 255, 255, 255, int(fade*255), TEXT_CENTERED, -1, -1,
						game::player1->impulse[IM_METER] > 0 ? (impulselength-game::player1->impulse[IM_METER] > impulsecost ? "\fy" : "\fw") : "\fg",
							int(len*100));
					popfont();
				}
				sy += sw;
