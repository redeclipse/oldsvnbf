		...
		switch(e.type)
		{
			...
			case SUNLIGHT:
			{
				if(showentdir >= level)
				{
					...
					part_dir(lightposition(e, vec(1, 1, 1).mul(getworldsize()/2)), e.attr[0], e.attr[1], getworldsize()*2, 1, colour);
					part_dir(lightposition(e), e.attr[0], e.attr[1], getworldsize()*2, 1, colour);
				}
				break;
			}
			case ACTOR:
