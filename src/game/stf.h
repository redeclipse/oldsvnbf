    ...
    void rendertether(gameent *d)
    {
        ...
        if(d->state==CS_ALIVE)
        {
            loopv(flags)
            {
                ...
                {
					part_trail(PART_PLASMA, 1, cl.feetpos(d, 1.f), vec(b.pos).sub(vec(0, 0, 4.f)), teamtype[d->team].colour, 4.8f);
					regularshape(PART_PLASMA, (int)d->height, teamtype[d->team].colour, 53, 3, 50, cl.feetpos(d, 1.f), 4.f);
					d->lastflag = i;
                }
            }
        }
