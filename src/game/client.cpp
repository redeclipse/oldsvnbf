		...
		while(p.remaining())
		{
			...
			switch(type)
			{
				...
				case SV_SHOTFX:
				{
					...
					s->setweapstate(weap, WEAP_S_SHOOT, WPB(int, weap, adelay, flags&HIT_ALT), lastmillis);
					s->weapshot[weap] = WPB(int, weap, sub, flags&HIT_ALT);
					s->totalshots += int(WPB(int, weap, damage, flags&HIT_ALT)*damagescale)*WPB(int, weap, rays, flags&HIT_ALT);
