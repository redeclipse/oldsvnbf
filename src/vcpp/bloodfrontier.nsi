...
OutFile "bloodfrontier_2006_xx_xx_setup.exe"
...
Section "Start Menu Shortcuts"
  ...
  CreateShortCut "$INSTDIR\BloodFrontier.lnk"                "$INSTDIR\bin\bloodfrontier.exe" "-r" "$INSTDIR\bin\bloodfrontier.exe" 0
  CreateShortCut "$SMPROGRAMS\BloodFrontier\BloodFrontier.lnk" "$INSTDIR\bin\bloodfrontier.exe" "-r" "$INSTDIR\bin\bloodfrontier.exe" 0
