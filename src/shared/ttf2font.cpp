...
void ttf2font(const char *name, int size, int pt, int pad, int shadow, int quality)
{
	string n;
				...
				if(!fail)
				{
					...
					if(!fail)
					{
						...
						loopk(shadow ? 2 : 1)
						{
							...
							SDL_Rect r = { a.x+(w?shadow:0), a.y+(w?shadow:0), 0, 0 };
...
int main(int argc, char *argv[])
{
	if(argc > 1)
	{
		...
		if(SDL_Init(SDL_INIT_NOPARACHUTE) != -1)
		{
			if(TTF_Init() != -1)
			{
				ttf2font(argv[1], argc > 2 ? atoi(argv[2]) : 512, argc > 3 ? atoi(argv[3]) : 68, argc > 4 ? atoi(argv[4]) : 1, argc > 5 ? atoi(argv[5]) : 4, argc > 6 ? atoi(argv[6]) : 1);
				TTF_Quit();
			}
